...
class Viewcheck extends IndexBase
{
    ...
    public function ckmoney($md5str=''){
        ...
        }elseif( !table_field(config('system_dirname')."_content1" , 'buyuser') ){
            into_sql("ALTER TABLE  `qb_".config('system_dirname')."_content1` ADD  `buyuser` TEXT NOT NULL COMMENT  '消费积分才能浏览的用户'");
        }
        $info = ContentModel::getInfoByid($id);
