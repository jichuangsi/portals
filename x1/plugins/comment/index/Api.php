...
//      }
     ...
     * 获取我的评论记录
     */
    public function get_ss($rows=0,$status=1,$order='',$by='',$page=0,$uid=1){
        ...
        $min = ($page-1)*$rows;
        $listds = contentModel::where($map)->order($order)->paginate($rows);
        if(!is_object($listds)){
            return $listds;
        }
        $listds->each(function($rs,$key){
     ...
     * @return unknown
     */
    public function get_list($sysid=0,$aid=0,$rows=0,$status=1,$order='',$by='',$page=0){
        if(self::$get_children!==null){  //取引用回复
            $map = [
                    'pid'=>self::$get_children,
            ];
